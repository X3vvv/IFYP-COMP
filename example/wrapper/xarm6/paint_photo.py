...
def canny_edge():
    ...
    resized = cv2.resize(img_cropped, None, fx=0.3, fy=0.3, interpolation=cv2.INTER_AREA)
    ...
    for contour in contours:
        ...
        for point in contour[1:]:
            x, y = point[0]
            gcode.append(f'G1 X{x+100} Y{y+60} Z237 F100')  # Cutting command with depth of cut and feed rate
...
def draw_gcode():
    ...
    # Top-Left coordinate(0,0) to (2,8) reference point
        if not params['quit']:
            ...
            params['variables']['Offset_x'] = (params['variables'].get('Center_x', 0) * -40)
            params['variables']['Offset_y'] = (params['variables'].get('Center_y', 0) * -25)
            arm.set_world_offset([params['variables'].get('Offset_x', 0), params['variables'].get('Offset_y', 0), 0, 0, 0, 0])  # x(-40) y(-20) z
            arm.set_state(0)
            time.sleep(0.5)
        if arm.error_code == 0 and not params['quit']:
            code = arm.set_position(*[180.0, -105.0, 270.3, -178.8, -1.1, -43.6], speed=params['speed'], mvacc=params['acc'], radius=-1.0, wait=True)
            if code != 0:
                params['quit'] = True
                pprint('set_position, code={}'.format(code))
        if arm.error_code == 0 and not params['quit']:
            code = arm.set_position(*[180.0, -105.0, 237.0, -178.8, -1.1, -43.6], speed=params['speed'], mvacc=params['acc'], radius=-1.0, wait=True)
            if code != 0:
                params['quit'] = True
                pprint('set_position, code={}'.format(code))
        ...
        if not params['quit']:
            #here
            arm.run_gcode_file(path='output.nc', speed=1000)
